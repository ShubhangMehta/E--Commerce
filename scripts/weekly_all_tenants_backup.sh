...
echo "🚀 Starting WEEKLY tenant backups for $DB_NAME on $DATE"

# Fetch tenant schemas (exclude system schemas)
TENANT_SCHEMAS=$(psql \
...
for schema in $TENANT_SCHEMAS; do
  ...
  # Dump schema & upload to Supabase → all_tenants
  pg_dump \
    "host=$DB_HOST port=$DB_PORT dbname=$DB_NAME user=$DB_USER password=$DB_PASSWORD sslmode=$DB_SSLMODE" \
    -n "$schema" -Fc \
  | /Users/sasiabburi/E--Commerce/venv/bin/python3 \
        /Users/sasiabburi/E--Commerce/backups/utils/upload_to_supabase.py \
        "$schema" "$DATE" "weekly" "all_tenants"

  echo "✅ Uploaded: $schema.dump → tenants/weekly/$DATE/all_tenants/"
done

echo "🎉 All tenant WEEKLY backups completed successfully!"